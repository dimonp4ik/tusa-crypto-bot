...
def get_symbol_performance(symbol: str, lookback: int = None) -> dict:
    ...
    with _conn() as c:
        rows = c.execute(
            f"SELECT status, realized_r, size_mult FROM signals WHERE symbol = ? AND status IN ({placeholders})"
            f" AND opened_at >= ?"
...
def get_symbols_performance(days: int = 30, since_ts: float = None) -> list:
    ...
    with _conn() as c:
        rows = c.execute(
            f"SELECT symbol, status, realized_r, size_mult FROM signals "
        ...
        cur = c.execute("""
            INSERT INTO setup_log
                (ts, symbol, direction, entry_price, tp1, tp2, sl,
                 ...
                 hours_to_event, next_event, sniper, funding_rate, signal_bar_ts)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                    ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            time_mod.time(),
...
def get_setup_accuracy(since_ts: float) -> dict:
    ...
    2026-07-22): what if we'd taken the OPPOSITE direction on each rejected setup
    with the levels swapped (original SL becomes the mirror's TP1, original TP1
    becomes the mirror's stop)? This is a near-clean inverse computable from the
    already-resolved outcome, no new simulation needed:
      - original hit SL (price ran 1R against it) → mirror reaches its TP1 (+1R)
      - original reached TP1 (price ran +0.7R for it) → mirror hits its stop (-0.7R)
      - original EXPIRED (no decisive move) → mirror also flat (~0)
    Mirror is +EV only when the rejected SL-rate clears ~41% (0.7/1.7); watch it
    populate on live data before trusting it (small samples, first-order R est,
    ignores mirror trailing upside + live fees/slippage on a tight 0.7R stop).
    """
    out = {"sent": {}, "rejected": {}}
    with _conn() as c:
...
def get_calibration_rows(since_ts: float, limit: int = 800) -> list:
    ...
    window otherwise reaches into the pre-parity-fix bot, mixing outcomes
    produced by software that no longer exists into the calibration of scales
    he is being asked to apply today."""
...
def at_has_open_position(user_id: int, inst_id: str) -> bool:
    ...
    along with it. Guard the entry instead of discovering this after filling.
    """
    with _conn() as c:
        row = c.execute("""
