...
def _open_for_user_locked(u: dict, sig: dict, inst_id: str, disp: str) -> None:
    ...
    try:
        ...
        budget_stop = original_sl
        if STOP_CLOSE_CONFIRM:
            distance = abs(planned-original_sl)*max(1.,STOP_EXCHANGE_BACKSTOP_R)
            budget_stop = px-distance if direction == 'LONG' else px+distance
        open_risk = 0.
        for position in at_all_open_positions():
            if position['user_id'] != uid:
                continue
            if not position.get('sl_algo_id'):
                raise ValueError('unprotected position pending emergency close')
            pspec = okx.get_xperp_spec(position['inst_id'])
            if not pspec:
                raise ValueError("open-position contract specification unavailable")
            open_risk += position_risk(position, pspec['ctVal'], AUTOTRADE_COST_RESERVE)
        margin = bounded_margin(requested=margin, equity=balance, leverage=AUTOTRADE_LEVERAGE,
            entry=px, stop=budget_stop, direction=direction, open_risk=open_risk,
            trade_fraction=AUTOTRADE_RISK_PER_TRADE, portfolio_fraction=AUTOTRADE_MAX_OPEN_RISK,
            cost_fraction=AUTOTRADE_COST_RESERVE)
    ...
    try:
        ...
        # Recheck the monetary budget at the worst displayed fill, never increase size.
        executable_stop = original_sl
        if STOP_CLOSE_CONFIRM:
            executable_stop = executable-distance if direction == 'LONG' else executable+distance
        allowed_margin = bounded_margin(requested=margin, equity=balance,
            leverage=AUTOTRADE_LEVERAGE, entry=executable, stop=executable_stop,
            direction=direction, open_risk=open_risk,
            trade_fraction=AUTOTRADE_RISK_PER_TRADE,
            portfolio_fraction=AUTOTRADE_MAX_OPEN_RISK, cost_fraction=AUTOTRADE_COST_RESERVE)
        sz = min(sz, okx.calc_contracts(allowed_margin, AUTOTRADE_LEVERAGE, executable, spec))
        if sz <= 0:
            raise ValueError("executable price leaves insufficient risk budget")
        px = quote.average
        ...
        try:
            ...
            if new_status in _CLOSE_STATUSES:
                ...
                try:
                    ...
                    # not the signal's global-feed entry. Off by the basis
                    # otherwise, which on a 10x position is not cosmetic.
                ...
                if pos.get("tp1_algo_id"):
                    try:
                        ...
                        if ok and real_sz < float(pos["sz"]):
                            ...
                            # Say how much actually closed. The exchange fills
                            # in whole lots, so the realised share is rarely the
                            # configured one — report the measured difference
                            # rather than repeating the setting back.
                            _orig = float(pos["sz"])
...
def poll_exchange_closes() -> None:
    ...
    for pos in positions:
        try:
            ...
            # Two ways OKX can report 0 for a position that is really open:
            # a brand-new entry not yet visible on /account/positions, and a
            # transient empty payload. Either one would strip the stop off a
            # live position and stop tracking it — the worst outcome the
            # autotrader can produce. Hence: ignore young positions, then
            # require a second, independent read to agree.
