...
from config import SMC_WICK_REJECT_FRAC, SMC_STRONG_BODY_FRAC, TP_LEVEL_MIN_DISTANCE_PCT
from config import SL_REF_LOOKBACK
from config import SMC_FVG_NEAR_TOL, SMC_FVG_APPROACH_TOL, SMC_OB_NEAR_TOL, SMC_OB_APPROACH_TOL
...
def calculate_stoch_rsi(closes: list, rsi_period: int = 14,
    ...
    # of the entire SMC analysis in here, and that analysis dominates publish
    # latency, which is what makes the live entry land later than the model's.
    #
    ...
    # prefix-length) cases in each bot before shipping.
    #
    ...
    #
    # detect_rsi_divergence below builds a series the same way, but over a
    # window of ~lookback+14 bars rather than the full history, and profiling
    # puts it at 1.2% of the analysis. Left alone deliberately.
