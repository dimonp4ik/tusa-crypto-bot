...
PAUSE=YES ONLY for: confirmed exchange hack >$500M, total government crypto ban (US/EU/China), active major war start, exchange insolvency/collapse (FTX-scale).
...
_PAUSE_REQUIRED_KEYWORDS = {
    "hack", "хак", "взлом", "взломан", "украли", "украдено",
    "ban", "бан", "запрет", "запрещён", "запрещена",
    "war", "война", "войну", "вторжение",
    "collapse", "insolvency", "банкротство", "неплатёжеспособ",
    "крах биржи", "биржа закрыт", "биржа обанкрот",
    "exchange hack", "exchange collapse",
}
...
def get_day_events(max_events: int = 10) -> dict:
    ...
    for ev in root.iter("event"):
        ...
        if p["all_day"] or p["when_utc"] is None:
            # now_et never existed; an all-day event took the day-news button
            # down with NameError. The target day is picked in Riga time above.
