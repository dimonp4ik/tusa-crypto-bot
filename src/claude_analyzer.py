...
- BT2022+[...]: historical BASE RATE for entries like this one — the same rule-filter replayed over 2022→present price data, same format as Hist. This is a prior from past market regimes, not your own verdicts: it answers "how do entries of this shape usually resolve on this symbol". IMPORTANT: this span includes the 2022 Luna/FTX bear-crash regime mixed with later bull/range regimes — a single aggregate number can hide a regime-dependent split, so always check the per-trend breakdown (bear/bull/range) rather than the headline count, and weigh the bucket matching the current Str= field. When live Hist and BT2022+ disagree, trust live Hist — it reflects the current regime.
