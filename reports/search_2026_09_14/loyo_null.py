...
import numpy as _np; _rng = _np.random.default_rng(12345)
...
del rows
# the control: outcomes shuffled WITHIN each side, so the geometry, the costs, the calendar and
# every feature stay exactly as they are and only the link between condition and result is cut.
# Whatever survives here is what the filter lets through by luck.
for _s in (True, False):
    _m = np.flatnonzero(lg == _s)
    R[_m] = R[_rng.permutation(_m)]
print('  ИСХОДЫ ПЕРЕМЕШАНЫ — это контроль', flush=True)
...
pickle.dump(out, open('loyo_null_survivors.pkl', 'wb'))
