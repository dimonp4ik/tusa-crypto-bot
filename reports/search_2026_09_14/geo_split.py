...
BANK_G = (3.0, 1.0, 48)
PULL_GS = [(3.0, 1.0, 48), (3.0, 2.0, 24), (3.0, 1.5, 24), (3.0, 1.5, 12), (4.0, 2.0, 24),
           (3.0, 1.0, 24), (2.0, 1.5, 12)]
print('', flush=True)
print('  банк всегда 3/1/48ч; откат на своей геометрии', flush=True)
print('  откат: стоп тейк удерж | сделок/год  ВР    ср R  | $ DD12%% (риск)  | $ при 1.4%% живой размер, просадка | по годам (DD12%%)', flush=True)
RES = {}
for g in PULL_GS:
    tr = book_split(BANK_G, g)
    v = np.array([x[2] for x in tr])
    months = (tr[-1][1] - tr[0][0]) / (365.25 * 86400 / 12)
    rk, m = sim_w.money_at_dd(eqrisk(tr), 0.12)
    r14 = sim_w.simulate(live(tr), 0.014)
    per = []
    for y in range(2022, 2027):
        sub = [x for x in tr if YT[y] <= x[0] < YT[y + 1]]
        per.append(sim_w.money_at_dd(eqrisk(sub), 0.12)[1] if len(sub) >= 50 else float('nan'))
    RES[g] = (tr, per)
    print('        %3.1f  %3.1f  %3dч  |  %5.0f    %4.1f%%  %+.3f | $%6.0f (%.2f%%) | $%6.0f %5.1f%% | %s%s'
          % (g[0], g[1], g[2], len(v) / (months / 12), 100 * np.mean(v > 0), v.mean(), m, 100 * rk,
             r14['eq'], 100 * abs(L.dd_of(r14['curve'])),
             ' '.join('%4.0f' % x if np.isfinite(x) else '   -' for x in per),
             '  <- сейчас' if g == (3.0, 1.0, 48) else ''), flush=True)

print('', flush=True)
print('  -- слепой выбор геометрии отката ПО ДЕНЬГАМ: 4 года выбор, 5-й замер --', flush=True)
wins = seen = 0
for y in range(2022, 2027):
    lo, hi = YT[y], YT[y + 1]
    best, bk = -1, None
    for g, (tr, per) in RES.items():
        m = sim_w.money_at_dd(eqrisk([x for x in tr if not (lo <= x[0] < hi)]), 0.12)[1]
        if m > best:
            best, bk = m, g
    mp = RES[bk][1][y - 2022]
    mb = RES[(3.0, 1.0, 48)][1][y - 2022]
