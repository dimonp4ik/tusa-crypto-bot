...
BANK_G = (3.0, 1.0, 48)
CUR = (3.0, 1.0, 48)
CANDS = [CUR, (1.5, 1.5, 24), (2.0, 1.5, 24), (2.5, 1.5, 24), (3.0, 1.5, 24), (2.0, 1.5, 16),
         (2.0, 1.5, 12), (1.5, 2.0, 24), (2.0, 1.0, 24), (1.5, 1.0, 24)]
RES = {g: book_split(BANK_G, g) for g in CANDS}


def mlive(tr):
    return sim_w.money_at_dd(live(tr), 0.12)[1]


print('', flush=True)
print('  === слепой выбор геометрии отката ПОД ЖИВЫМ РАЗМЕРОМ: 4 года выбор, 5-й замер ===', flush=True)
wins = seen = 0
picks = collections.Counter()
for y in range(2022, 2027):
    lo, hi = YT[y], YT[y + 1]
    best, bk = -1, None
    for g, tr in RES.items():
        m = mlive([x for x in tr if not (lo <= x[0] < hi)])
        if m > best:
            best, bk = m, g
    mp = mlive([x for x in RES[bk] if lo <= x[0] < hi])
    mb = mlive([x for x in RES[CUR] if lo <= x[0] < hi])
    seen += 1; wins += int(mp > mb); picks[bk] += 1
    r1 = sim_w.simulate(live([x for x in RES[bk] if lo <= x[0] < hi]), 0.014)['eq']
    r0 = sim_w.simulate(live([x for x in RES[CUR] if lo <= x[0] < hi]), 0.014)['eq']
    print('    %d выбрано %s | слепой год DD12%%: $%.0f против $%.0f %s | при 1.4%%: $%.0f против $%.0f'
          % (y, bk, mp, mb, 'лучше' if mp > mb else ('так же' if mp == mb else 'хуже'), r1, r0), flush=True)
print('    лучше нынешней в %d из %d лет; выбиралось: %s' % (wins, seen, dict(picks)), flush=True)
