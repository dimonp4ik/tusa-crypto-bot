...
SIG = []   # (close, prio, key, coin, e, atr, Mu, Md, on, cn, tt, slip)
for s in COINS:
    c = SP.CTX[s]
...
cnt = collections.Counter(x[2] for x in SIG)
print('  сигналов: %s' % ', '.join('%s %d' % kv for kv in sorted(cnt.items())), flush=True)
for k in range(5):
    print('    r%d = %s' % (k, RULES[k]['name']), flush=True)
...
def m_dd(rows):
    return sim_w.money_at_dd(rows, 0.12)[1] if len(rows) >= 80 else float('nan')
...
M0 = m_dd(base)
F0, D0 = m_fix(base)
print('  опора: %d сделок, DD12%% $%.0f, при 1.4%% $%.0f просадка %.1f%%' % (len(base), M0, F0, 100 * D0), flush=True)

GRID = list(itertools.product((2.0, 3.0, 4.0), (0.75, 1.0, 1.5, 2.0), (24, 48, 72)))
FINAL = {}
for k in range(5):
    key = 'r%d' % k
    res = {}
    for g in GRID:
        rows = book({key: g})
        res[g] = rows
    ranked = sorted(res, key=lambda g: -m_dd(res[g]))
    print('', flush=True)
    print('  ===== %s (%s) =====' % (key, RULES[k]['name']), flush=True)
    for g in ranked[:5] + ([BASE_G] if BASE_G not in ranked[:5] else []):
        mf, dd = m_fix(res[g])
        print('    стоп %.0f тейк %.2f %2dч | DD12%% $%6.0f (%+.0f%%) | 1.4%% $%6.0f %4.1f%%%s'
              % (g[0], g[1], g[2], m_dd(res[g]), 100 * (m_dd(res[g]) / M0 - 1), mf, 100 * dd,
                 '  <- сейчас' if g == BASE_G else ''), flush=True)
    wins = seen = 0
    for y in range(2022, 2027):
        lo, hi = YT[y], YT[y + 1]
        best, bk = -1, None
        for g, rows in res.items():
            m = m_dd([x for x in rows if not (lo <= x[0] < hi)])
            if np.isfinite(m) and m > best:
                best, bk = m, g
        mp = m_dd([x for x in res[bk] if lo <= x[0] < hi])
        mb = m_dd([x for x in res[BASE_G] if lo <= x[0] < hi])
        if not (np.isfinite(mp) and np.isfinite(mb)):
            continue
        seen += 1; wins += int(mp > mb)
        print('      слепо %d: выбрано %s, $%.0f против $%.0f %s' % (y, bk, mp, mb,
              'лучше' if mp > mb else ('так же' if mp == mb else 'хуже')), flush=True)
    print('    слепой выбор лучше нынешней в %d из %d лет' % (wins, seen), flush=True)
    FINAL[key] = (ranked[0], wins, seen)

print('', flush=True)
print('  === итог по правилам ===', flush=True)
for key, (g, w, n) in FINAL.items():
    print('    %s лучшая на всей истории %s, слепо %d/%d' % (key, g, w, n), flush=True)
