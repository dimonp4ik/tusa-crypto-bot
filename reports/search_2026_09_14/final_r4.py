...
GEOS = (('сейчас (r4 стоп 3 / тейк 1)', {}), ('НОВОЕ (r4 стоп 2 / тейк 1.5)', {'r4': (2.0, 1.5, 48)}))
...
def summary(rows):
    ...
    return dict(tpy=len(v) / (months / 12), wr=float(np.mean(v > 0)), eq=r['eq'],
                mo=(r['eq'] / 120) ** (1 / months) - 1, yr=(r['eq'] / 120) ** (12 / months),
                dd=abs(L.dd_of(r['curve'])), latch=bool(r['paused_at']))


for book_name, coins in BOOKS:
    sig = make_sig(coins)
    print('', flush=True)
    print('  ===== %s: банк + откат 1ч + откат 2ч, риск 1.4%% =====' % book_name, flush=True)
    for lbl, geo in GEOS:
        m = summary(book(sig, geo))
        print('    %-30s 100%% заливки: %4.0f сд/год ВР %.1f%% %+.2f%%/мес x%.2f/год $%.0f просадка %.1f%%'
              % (lbl, m['tpy'], 100 * m['wr'], 100 * m['mo'], m['yr'], m['eq'], 100 * m['dd']), flush=True)
        res = []
        for seed in range(1, 11):
            rng = np.random.default_rng(seed)
            sub = [x for x in sig if rng.random() < 0.85]
            res.append(summary(book(sub, geo)))
        med = lambda k: float(np.median([x[k] for x in res]))
        print('    %-30s  85%% заливки: %4.0f сд/год ВР %.1f%% %+.2f%%/мес x%.2f/год $%.0f просадка %.1f%% (макс %.1f%%, защёлок %d/10)'
              % ('', med('tpy'), 100 * med('wr'), 100 * med('mo'), med('yr'), med('eq'), 100 * med('dd'),
                 100 * max(x['dd'] for x in res), sum(1 for x in res if x['latch'])), flush=True)
