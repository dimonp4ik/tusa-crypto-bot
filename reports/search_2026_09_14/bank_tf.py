...
def bank_sec(s, sec):
    """The bank's own five rules evaluated on `sec` bars, exactly as hourly_signals does:
    regime interval and side must match, BTC SMA(50) daily filter on longs, first match wins."""
    c = SP.CTX[s]
    T, B = PB.build_bars(c['t15'], c['a15'], sec)
    F = PB.features(T, B, BTC['t15'], BTC['a15'])
    iv, starts = c['iv'], c['starts']
    bdt, bdc = c['bdt'], np.asarray(c['bdc'], dtype=float)
    sma = PB._sma(bdc, 50)
    hold = min(48 * sec, 72 * 3600) // 900
    out = []
    for i in range(PB.MIN_HOURS // max(1, sec // 3600), len(T)):
        if not np.isfinite(F['atr'][i]):
            continue
        close = int(T[i]) + sec
        j = bisect.bisect_right(starts, close) - 1
        if j < 0 or not (iv[j][0] <= close < iv[j][1]):
            continue
        side = iv[j][2]
        if side == 'LONG':
            d = np.searchsorted(bdt, close - 86400, side='right') - 1
        ...
        for r in RULES:
            if r.get('side', 'LONG') == side and PB._match(r, F, i):
                out.append((close, s, side == 'LONG', float(F['atr'][i]), r['sl'], 1.0, hold))
                break
    return out
...
def make_sources(coins):
    ...
    src['банк 2ч'] = [x for s in coins for x in bank_sec(s, 7200)]
    return src
...
for book_name, coins in (('КНИГА 1 (без XLM, AAVE)', [c for c in SP.COINS if c not in ('BILLUSDT', 'XLMUSDT', 'AAVEUSDT')]),
    ...
    SOURCES = make_sources(coins)
    globals()['SOURCES'] = SOURCES
    print('', flush=True)
    print('  ===== %s: сигналов %s =====' % (book_name, ', '.join('%s %d' % (k, len(v)) for k, v in SOURCES.items())), flush=True)
    report('банк 2ч отдельно', build(['банк 2ч']))
    PER = {}
    for names in (['банк 1ч', 'откат 1ч', 'откат 2ч'], ['банк 1ч', 'откат 1ч', 'откат 2ч', 'банк 2ч'],
                  ['банк 1ч', 'банк 2ч'], ['банк 1ч']):
        rows = build(names)
        report(' + '.join(names), rows)
        plain = [x[:6] for x in rows]
        PER[tuple(names)] = [sim_w.simulate([x for x in plain if YT[y] <= x[0] < YT[y + 1]], 0.014)['eq']
                             for y in range(2022, 2027)]
        cnt = collections.Counter(x[6] for x in rows)
        print('      сделок по источникам: %s' % ', '.join('%s %d' % (names[p], n) for p, n in sorted(cnt.items())), flush=True)
    a = PER[('банк 1ч', 'откат 1ч', 'откат 2ч')]
    b = PER[('банк 1ч', 'откат 1ч', 'откат 2ч', 'банк 2ч')]
    print('  +банк 2ч к стеку: лучше в %d из 5 лет (%s)'
          % (sum(1 for x, y in zip(a, b) if y > x), ' '.join('%+.0f' % (y - x) for x, y in zip(a, b))), flush=True)
