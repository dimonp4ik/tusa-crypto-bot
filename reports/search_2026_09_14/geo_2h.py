...
BASE_GEO = {'r4': (2.0, 1.5, 48)}          # the new system: short rule already on its own geometry
...
GRID = [(sl, tp, hh) for sl in (2.0, 3.0, 4.0) for tp in (0.75, 1.0, 1.5, 2.0) for hh in (36, 48, 72)]
CUR = (3.0, 1.0, 72)

for bi, (book_name, coins) in enumerate(BOOKS):
    sig = make_sig(coins)
    base = book(sig, BASE_GEO)
    bf, bd = m_fix(base)
    bpy = per_year(base)
    print('', flush=True)
    print('  ===== %s: откат 2ч на своей геометрии (база: новая система, $%.0f при 1.4%%, просадка %.1f%%) ====='
          % (book_name, bf, 100 * bd), flush=True)
    RES = {}
    for g in GRID:
        geo = dict(BASE_GEO); geo['о2ч'] = g
        RES[g] = book(sig, geo)
    ranked = sorted(RES, key=lambda g: -m_fix(RES[g])[0])
    for g in ranked[:6] + ([CUR] if CUR not in ranked[:6] else []):
        mf, dd = m_fix(RES[g])
        py = per_year(RES[g])
        own = np.array([x[2] for x in RES[g] if x[6] == 'о2ч'])
        print('    о2ч %s | 1.4%%: $%6.0f %4.1f%% | DD12%%: $%6.0f | свои %3d ВР %4.1f%% ср %+.3f | лучше базы по годам %d/5%s'
              % (g, mf, 100 * dd, m_dd(RES[g]), len(own), 100 * np.mean(own > 0) if len(own) else 0,
                 own.mean() if len(own) else 0, sum(1 for a, b in zip(bpy, py) if b > a),
                 '  <- сейчас' if g == CUR else ''), flush=True)
    if bi == 0:
        print('  -- слепой выбор геометрии о2ч (мера: $ при 1.4%%, живой размер): 4 года выбор, 5-й замер --', flush=True)
        wins = seen = 0
        for y in range(2022, 2027):
            ...
            mb = sim_w.simulate([x for x in plain(RES[CUR]) if lo <= x[0] < hi], 0.014)['eq']
            seen += 1; wins += int(mp > mb)
            print('    %d выбрано %s | слепой год $%.0f против нынешней $%.0f %s'
