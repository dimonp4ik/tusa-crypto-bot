...
rows = build(['банк 1ч', 'откат 1ч', 'откат 2ч'])
print('', flush=True)
print('  === книга банк 1ч + откат 1ч + откат 2ч: разбор по дню недели входа (UTC) ===', flush=True)
print('  день  сделок  ВР     ср R    | ср R по годам 2022..2026            | лет в плюсе', flush=True)
tab = collections.defaultdict(list)
for x in rows:
    d = datetime.datetime.fromtimestamp(x[0], datetime.UTC)
    tab[(d.weekday(), d.year)].append(x[2])
allR = np.mean([x[2] for x in rows])
for wd in range(7):
    v = np.array([r for (w, y), vals in tab.items() if w == wd for r in vals])
    per, pos, seen = [], 0, 0
    for y in range(2022, 2027):
        vy = tab.get((wd, y), [])
        if len(vy) >= 15:
            m = float(np.mean(vy)); per.append('%+.3f' % m); seen += 1; pos += int(m > 0)
        else:
            per.append('  -   ')
    print('  %s   %5d  %4.1f%%  %+.4f  | %s | %d/%d'
          % (DAYS[wd], len(v), 100 * np.mean(v > 0), v.mean(), ' '.join(per), pos, seen), flush=True)
print('  среднее по книге %+.4f' % allR, flush=True)
