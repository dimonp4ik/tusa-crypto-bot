...
print('', flush=True)
print('  === каждый источник отдельно ===', flush=True)
for nm in SOURCES:
    report(nm, build([nm]))
print('', flush=True)
print('  === стек в одном счёте (приоритет слева направо) ===', flush=True)
for names in (['банк 1ч'], ['банк 1ч', 'откат 1ч'], ['банк 1ч', 'откат 1ч', 'откат 2ч'],
              ['банк 1ч', 'откат 1ч', 'откат 4ч'], ['банк 1ч', 'откат 1ч', 'откат 2ч', 'откат 4ч'],
              ['банк 1ч', 'откат 2ч'], ['банк 1ч', 'откат 4ч']):
    rows = build(names)
