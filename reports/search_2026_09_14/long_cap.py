...
def build_cap(names, cap_long):
    """Same as build(), plus: a LONG is refused while cap_long longs are already open."""
    ev = []
    for pr, nm in enumerate(names):
        ev += [(t, pr, s, lg, a, sl, tp, hd) for t, s, lg, a, sl, tp, hd in SOURCES[nm]]
    ev.sort()
    ...
    for t, pr, s, lg, a, sl, tp, hd in ev:
        if busy.get(s, 0) > t:
            continue
        open_long = [e for e in open_long if e > t]
        if lg and cap_long is not None and len(open_long) >= cap_long:
            continue
        c = SP.CTX[s]
        j = c['pos'].get(t)
        if j is None:
...
for book_name, coins in (('КНИГА 1 (без XLM, AAVE)', [c for c in SP.COINS if c not in ('BILLUSDT', 'XLMUSDT', 'AAVEUSDT')]),
    ...
    for cap in (None, 8, 6, 5, 4, 3):
        rows = build_cap(NAMES, cap)
        report('потолок лонгов %s' % ('нет' if cap is None else cap), rows)
        plain = [x[:6] for x in rows]
        ...
        if base_py is None:
            ...
            print('      против опоры по годам: лучше в %d из 5 (%s)'
                  % (sum(1 for x, y in zip(base_py, py) if y > x), ' '.join('%+.0f' % (y - x) for x, y in zip(base_py, py))), flush=True)
