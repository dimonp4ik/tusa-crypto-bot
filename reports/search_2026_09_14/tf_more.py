...
def make_sources(coins):
    ...
    for sec, nm in ((3600, 'откат 1ч'), (5400, 'откат 90м'), (7200, 'откат 2ч'), (10800, 'откат 3ч')):
...
def per_year(rows):
    plain = [x[:6] for x in rows]
    return [sim_w.simulate([x for x in plain if YT[y] <= x[0] < YT[y + 1]], 0.014)['eq'] for y in range(2022, 2027)]


BASE_STACK = ['банк 1ч', 'откат 1ч', 'откат 2ч']
for book_name, coins in (('КНИГА 1 (без XLM, AAVE)', [c for c in SP.COINS if c not in ('BILLUSDT', 'XLMUSDT', 'AAVEUSDT')]),
    ...
    globals()['SOURCES'] = make_sources(coins)
    print('', flush=True)
    print('  ===== %s: сигналов %s =====' % (book_name, ', '.join('%s %d' % (k, len(v)) for k, v in SOURCES.items())), flush=True)
    base_rows = build(BASE_STACK)
    report('опора: ' + ' + '.join(BASE_STACK), base_rows)
    by = per_year(base_rows)
    for extra in (['откат 90м'], ['откат 3ч'], ['откат 90м', 'откат 3ч']):
        names = BASE_STACK + extra
        rows = build(names)
        report('+ ' + ' + '.join(extra), rows)
        py = per_year(rows)
        cnt = collections.Counter(x[6] for x in rows)
        print('      лучше опоры в %d из 5 лет (%s) | сделок по источникам: %s'
              % (sum(1 for a, b in zip(by, py) if b > a), ' '.join('%+.0f' % (b - a) for a, b in zip(by, py)),
                 ', '.join('%s %d' % (names[p], n) for p, n in sorted(cnt.items()))), flush=True)
