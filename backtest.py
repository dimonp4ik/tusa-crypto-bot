...
def fetch_history(
    ...
    *,
    ...
    if not refresh_cache and path.exists():
        ...
        # A cache anchored to an explicit historical endpoint is immutable.
        # Expiring it after two hours wastes thousands of paginated requests
        # and can silently replace a research input between audit runs.  Only
        # the rolling "latest candles" cache needs a TTL.
        if end_date_ms is not None or age < CACHE_TTL_SEC:
            try:
